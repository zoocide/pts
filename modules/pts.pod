...
PTS accepts three different ways to select tasks to process:
...
In this case, PTS searches the specified name inside known task directories.
...
PTS calls this method for every task from the input task sequence before the start of the processing stage.
